...
tau_samples=c(out_ls$PT_chain[[1]][low:up,2],rep(1,length(out_ls$PT_chain[[1]][low:up,2])))
...
for (i in (1:20)){
out_ls=get(load(paste('mllik',i,'.RData',sep='')))
tau_samples[[i]]=c(out_ls$PT_chain[[1]][low:up,2],rep(1,length(out_ls$PT_chain[[1]][low:up,2])))
...
clusterCall(cl,function(x) {source("ST_functions_new.r")})
