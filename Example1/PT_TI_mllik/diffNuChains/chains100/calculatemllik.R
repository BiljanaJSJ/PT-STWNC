...
#path: /stat/stat2/bjonoska/ST/newST/quantileTrick/sampleVar7/paper/paper_lastrun/MarginalLlik_12112015/TI_mllik
...
#up=35000
#for (i in (1:20)){
#out_ls=runST(niter=35000,SigmaPriorPars=c(1,1),n=25,k=1,switchllik='power',q1=0.15,q2=0.1,kp1=c(1,1),kptau=1)
#save(out_ls,file=paste('mllik',i,'.RData',sep=''))
#mllik[i]=mean(out_ls$mllik[[1]][low:up])

#acfmllik=acf(out_ls$mllik[[1]][low:up])$acf
#acfmllik[which(acfmllik<0.05)]
#acfmllik=2*sum(acfmllik)-1
#mean_se[i]=sqrt(var(out_ls$mllik[[1]][low:up])/(up-low))*sqrt(acfmllik)
#
#}

##out_mllik=get(load('PT_mllik_out_ls.RData'))
#out_mllik=list()
#out_mllik[[1]]=mllik
#out_mllik[[2]]=mean_se
#out_mllik[[3]]=sqrt(var(mllik))

#save(out_mllik,file='mllik_new.RData')
